...
import com.his.utils.aliPayTools.AlipayNotify;
...
public class AlipayController {
	...
	@RequestMapping(value = { "/notify_pay" }, method = { RequestMethod.POST })
	@ResponseBody
...
//			valueStr = new String(valueStr.getBytes("ISO-8859-1"), "gbk");
		...
		if(notify_id!="" && notify_id!=null){
			System.out.println(AlipayNotify.verifyResponse(notify_id));
			if(AlipayNotify.verifyResponse(notify_id).equals("true"))//判断成功之后使用getResponse方法判断是否是支付宝发来的异步通知。
			{
				if(AlipayNotify.getSignVeryfy(params, sign))//使用支付宝公钥验签
				{
					if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
						//业务处理
						outpatientPayService.addAliPay(params);
					}
